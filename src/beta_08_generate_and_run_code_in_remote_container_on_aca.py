...
class RemoteExecutor(CodeExecutor):
    ...
    def execute_code_blocks(self, code_blocks: List[CodeBlock]) -> CodeResult:
        log = ""
        tool = SessionsPythonREPLTool(pool_management_endpoint="<TODO: GET SessionPool Management Endpoint from Env File>")
        for code_block in code_blocks:
            result = tool.invoke(code_block.code)
            log += result
        return CodeResult(exit_code=1, output=result)
