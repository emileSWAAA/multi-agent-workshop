...
# Code modified from original code:
# https://github.com/microsoft/semantic-kernel/blob/main/python/samples/concepts/mcp/servers/restaurant_booking_agent_server.py
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Run the Semantic Kernel MCP server."
    )
    parser.add_argument(
        "--transport",
        type=str,
...
class BookingPlugin:
    ...
    @kernel_function(
        description="Asks for a booking, will return 'confirmed' or 'denied'."
    )
    def book_a_table(
        self,
        restaurant: Annotated[
            Literal["The Farm, The Harbor, The Joint"],
...
async def run(
    transport: Literal["sse", "stdio"] = "stdio", port: int | None = None
) -> None:
    kernel = Kernel()
    service_id = "booking-agent"
